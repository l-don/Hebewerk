  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-[#F6F4EB] px-4 py-10 relative overflow-hidden font-body">
      ...
      <div class="w-full max-w-md notebook-card rounded-2xl p-6 sm:p-8 relative z-10 shadow-xl bg-white border-2 border-[#2D3748]">
        ...
        @if (successMessage()) {
          <div class="p-3 mb-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-800 text-xs font-body flex items-center gap-2">
            <span>✅</span>
        ...
        @if (isForgotPasswordMode()) {
          <form (ngSubmit)="handleResetPassword()" #resetForm="ngForm" class="space-y-4">
            <div>
            ...
            <p class="text-center text-xs font-body text-[#718096] mt-4">
              <a 
                href="javascript:void(0)" 
                (click)="toggleForgotPasswordMode(false)"
                class="highlighter-yellow font-heading font-bold text-[#1A1A1A] px-1.5 py-0.5 rounded"
              >
                ← Zurück zum Anmelden
              </a>
            </p>
          ...
          <form (ngSubmit)="onSubmit()" #authForm="ngForm" class="space-y-4">
            ...
            <div>
              <div class="flex justify-between items-center mb-1">
                ...
                @if (!isRegisterMode()) {
                  <a href="javascript:void(0)" (click)="toggleForgotPasswordMode(true)" class="text-xs text-[#718096] hover:text-black font-body">Vergessen?</a>
                }
              </div>
              <input 
                type="password" 
...
export class AuthComponent {
  ...
  toggleForgotPasswordMode(enable: boolean) {
    this.isForgotPasswordMode.set(enable);
    this.errorMessage.set(null);
    this.successMessage.set(null);
  }
  ...
  async handleResetPassword() {
    ...
    try {
      ...
      this.successMessage.set('Eine E-Mail zum Zurücksetzen des Passworts wurde versendet!');
      this.isForgotPasswordMode.set(false);
    } catch (err: any) {
      this.errorMessage.set(err.message || 'Zurücksetzen fehlgeschlagen. Bitte prüfe deine E-Mail-Adresse.');
    } finally {
